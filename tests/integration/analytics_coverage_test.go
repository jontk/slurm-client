...
// skipUnsupportedVersion skips the test if the version is not supported
func skipUnsupportedVersion(t *testing.T, version string) {
	if version == "v0.0.41" {
		t.Skip("v0.0.41 is not supported (uses inline schemas that oapi-codegen cannot generate)")
	}
}
...
func TestAnalyticsCoverage_JobUtilization(t *testing.T) {
	...
	for _, version := range versions {
		t.Run(version, func(t *testing.T) {
			skipUnsupportedVersion(t, version)
...
func TestAnalyticsCoverage_JobLiveMetrics(t *testing.T) {
	...
	for _, version := range versions {
		t.Run(version, func(t *testing.T) {
			...
			// Skip v0.0.40 - live metrics not supported
			if version == "v0.0.40" {
				t.Skip("Real-time job monitoring is not supported in API v0.0.40")
			}
...
func TestAnalyticsCoverage_JobResourceTrends(t *testing.T) {
	...
	for _, version := range versions {
		t.Run(version, func(t *testing.T) {
			...
			// Skip v0.0.40 - resource trends not supported
			if version == "v0.0.40" {
				t.Skip("Resource trend analysis is not supported in API v0.0.40")
			}
...
func TestAnalyticsCoverage_ResourceSpecificAnalytics(t *testing.T) {
	...
	for _, version := range versions {
		t.Run(version, func(t *testing.T) {
			...
			assert.GreaterOrEqual(t, cpuAnalytics.UtilizationPercent, 0.0)
			assert.LessOrEqual(t, cpuAnalytics.UtilizationPercent, 100.0)
			...
			assert.GreaterOrEqual(t, memoryAnalytics.UtilizationPercent, 0.0)
			assert.LessOrEqual(t, memoryAnalytics.UtilizationPercent, 100.0)
			...
			assert.GreaterOrEqual(t, ioAnalytics.ReadOperations, int64(0))
			assert.GreaterOrEqual(t, ioAnalytics.WriteOperations, int64(0))
			...
			assert.Equal(t, uint32(1001), comprehensive.JobID)
...
// Note: This test requires jobs to exist in the system, which mock servers don't provide
func TestAnalyticsCoverage_JobStepAnalytics(t *testing.T) {
	t.Skip("Job step analytics tests require real jobs in SLURM. Set SLURM_REAL_SERVER_TEST=true to test with real server")
...
func TestAnalyticsCoverage_IntegrationMethods(t *testing.T) {
	...
	for _, version := range versions {
		t.Run(version, func(t *testing.T) {
			...
			// Note: These methods may not be fully implemented in all versions
			// We test to ensure they exist and have the correct signatures
			...
			// May return error if not implemented, or nil if mock handles it
			if err != nil {
...
func TestAnalyticsCoverage_ContextCancellation(t *testing.T) {
	...
	_, err = client.Jobs().GetJobUtilization(ctx, "1001")
	assert.Error(t, err)
	// Context cancellation may manifest as different errors depending on timing
	// Just verify that an error occurred (context canceled, timeout, or connection error)
	assert.NotNil(t, err)
}
...
func TestAnalyticsCoverage_RealWorldScenarios(t *testing.T) {
	...
	t.Run("CompleteAnalysisWorkflow", func(t *testing.T) {
		...
		// Note: Different analytics methods may return different granularity of data
		// so we just verify the structures exist rather than exact value matching
		if utilization.CPUUtilization != nil && performance.ResourceUtilization != nil {
			assert.Greater(t, utilization.CPUUtilization.Allocated, 0.0)
			assert.Greater(t, performance.ResourceUtilization.Allocated, 0.0)
		}
		if utilization.MemoryUtilization != nil && performance.JobUtilization != nil &&
			performance.JobUtilization.MemoryUtilization != nil {
			assert.Greater(t, utilization.MemoryUtilization.Allocated, 0.0)
			assert.Greater(t, performance.JobUtilization.MemoryUtilization.Allocated, 0.0)
		}
