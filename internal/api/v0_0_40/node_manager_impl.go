...
func (m *NodeManagerImpl) List(ctx context.Context, opts *interfaces.ListNodesOptions) (*interfaces.NodeList, error) {
	...
	params := &SlurmV0040GetNodesParams{}

	// Set flags to get detailed node information
	flags := SlurmV0040GetNodesParamsFlagsDETAIL
	params.Flags = &flags

	// Call the generated OpenAPI client
	resp, err := m.client.apiClient.SlurmV0040GetNodesWithResponse(ctx, params)
	if err != nil {
...
// convertAPINodeToInterface converts a V0040Node to interfaces.Node
func convertAPINodeToInterface(apiNode V0040Node) (*interfaces.Node, error) {
	...
	// Partitions - V0040CsvString (which is []string)
	...
	// Features - V0040CsvString (which is []string)
	...
	// Last busy time - V0040Uint64NoValStruct
	...
	// Free Memory - V0040Uint64NoVal (in MB from API)
...
func (m *NodeManagerImpl) Get(ctx context.Context, nodeName string) (*interfaces.Node, error) {
	...
	// Prepare parameters for the API call
	params := &SlurmV0040GetNodeParams{}

	// Set flags to get detailed node information
	flags := SlurmV0040GetNodeParamsFlagsDETAIL
	params.Flags = &flags

	// Call the generated OpenAPI client
	resp, err := m.client.apiClient.SlurmV0040GetNodeWithResponse(ctx, nodeName, params)
	if err != nil {
		wrappedErr := errors.WrapError(err)
		return nil, errors.EnhanceErrorWithVersion(wrappedErr, "v0.0.40")
	}
...
func (m *NodeManagerImpl) Update(ctx context.Context, nodeName string, update *interfaces.NodeUpdate) error {
	...
	// Call the generated OpenAPI client
	resp, err := m.client.apiClient.SlurmV0040PostNodeWithResponse(ctx, nodeName, *nodeUpdate)
	if err != nil {
		wrappedErr := errors.WrapError(err)
...
// convertNodeUpdateToAPI converts interfaces.NodeUpdate to V0040UpdateNodeMsg
func convertNodeUpdateToAPI(update *interfaces.NodeUpdate) (*V0040UpdateNodeMsg, error) {
	if update == nil {
		return nil, errors.NewClientError(errors.ErrorCodeInvalidRequest, "Node update cannot be nil")
	}

	nodeUpdate := &V0040UpdateNodeMsg{}

	// Set node state if provided
	if update.State != nil {
		// Convert state string to V0040NodeStates array
	...
	if len(update.Features) > 0 {
		features := V0040CsvString(update.Features)
...
// Note: v0.0.40 API does not support native streaming/WebSocket node monitoring
...
func (m *NodeManagerImpl) Drain(ctx context.Context, nodeName string, reason string) error {
	return errors.NewNotImplementedError("Drain", "v0.0.40")
}
...
func (m *NodeManagerImpl) Resume(ctx context.Context, nodeName string) error {
	return errors.NewNotImplementedError("Resume", "v0.0.40")
}
