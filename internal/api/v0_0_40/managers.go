...
// JobManager implements JobManager interface for API version v0.0.40
...
func (j *JobManager) List(ctx context.Context, opts *ListJobsOptions) (*JobList, error) {
	...
	// v0.0.40 API has limited filtering options
	if opts != nil {
...
func (i *InfoManager) Ping(ctx context.Context) error {
	...
	_, err := i.client.apiClient.SlurmV0040GetPartitionsWithResponse(ctx, nil)
	if err != nil {
...
func convertJobFromAPI(apiJob V0040JobInfo) Job {
	job := Job{
...
func convertJobSubmissionToAPI(submission *JobSubmission) V0040JobSubmitReq {
	...
	if submission.TimeLimit > 0 {
		jobDesc.TimeLimit = &V0040Uint32NoVal{
			Set:    boolptr(true),
			Number: int64ptr(int64(submission.TimeLimit)),
		}
	}
...
func convertNodeFromAPI(apiNode V0040Node) Node {
	node := Node{
		Metadata: make(map[string]interface{}),
	}
...
func convertPartitionFromAPI(apiPartition V0040PartitionInfo) Partition {
	partition := Partition{
		Metadata: make(map[string]interface{}),
	}
