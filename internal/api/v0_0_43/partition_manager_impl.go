...
func (m *PartitionManagerImpl) List(ctx context.Context, opts *interfaces.ListPartitionsOptions) (*interfaces.PartitionList, error) {
	...
	params := &SlurmV0043GetPartitionsParams{}

	// Set flags to get detailed partition information
	flags := SlurmV0043GetPartitionsParamsFlagsDETAIL
	params.Flags = &flags

	// Call the generated OpenAPI client
	resp, err := m.client.apiClient.SlurmV0043GetPartitionsWithResponse(ctx, params)
	if err != nil {
...
func convertAPIPartitionToInterface(apiPartition V0043PartitionInfo) (*interfaces.Partition, error) {
	...
	// No denied groups field in V0043PartitionInfo
...
func (m *PartitionManagerImpl) Get(ctx context.Context, partitionName string) (*interfaces.Partition, error) {
	...
	// Prepare parameters for the API call
	params := &SlurmV0043GetPartitionParams{}

	// Set flags to get detailed partition information
	flags := SlurmV0043GetPartitionParamsFlagsDETAIL
	params.Flags = &flags

	// Call the generated OpenAPI client
	resp, err := m.client.apiClient.SlurmV0043GetPartitionWithResponse(ctx, partitionName, params)
	if err != nil {
		wrappedErr := errors.WrapError(err)
		return nil, errors.EnhanceErrorWithVersion(wrappedErr, "v0.0.43")
	}
...
func (m *PartitionManagerImpl) Update(ctx context.Context, partitionName string, update *interfaces.PartitionUpdate) error {
	...
	return errors.NewClientError(
		...
		"The v0.0.43 Slurm REST API does not support partition update operations. Partition configuration changes must be made through slurmctld configuration files and require admin privileges.",
	)
}

// Watch provides real-time partition updates through polling
// Note: v0.0.43 API does not support native streaming/WebSocket partition monitoring
