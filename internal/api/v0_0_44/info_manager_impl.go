...
func (m *InfoManagerImpl) Get(ctx context.Context) (*interfaces.ClusterInfo, error) {
	...
	pingResp, err := m.client.apiClient.SlurmV0044GetPingWithResponse(ctx)
	if err != nil {
	...
	if pingResp.StatusCode() != 200 {
		...
		if pingResp.JSON200 != nil {
			...
			if pingResp.JSON200.Errors != nil && len(*pingResp.JSON200.Errors) > 0 {
				...
				apiError := errors.NewSlurmAPIError(pingResp.StatusCode(), "v0.0.44", apiErrors)
				return nil, apiError.SlurmError
			}
		}

		// Fall back to HTTP error handling
		httpErr := errors.WrapHTTPError(pingResp.StatusCode(), responseBody, "v0.0.44")
		return nil, httpErr
	}
	...
	diagResp, diagErr := m.client.apiClient.SlurmV0044GetDiagWithResponse(ctx)
...
func (m *InfoManagerImpl) PingDatabase(ctx context.Context) error {
	...
	resp, err := m.client.apiClient.SlurmdbV0044GetConfigWithResponse(ctx)
	if err != nil {
		wrappedErr := errors.WrapError(err)
		return errors.EnhanceErrorWithVersion(wrappedErr, "v0.0.44")
	}
...
func (m *InfoManagerImpl) Stats(ctx context.Context) (*interfaces.ClusterStats, error) {
	...
	nodesResp, err := m.client.apiClient.SlurmV0044GetNodesWithResponse(ctx, nil)
