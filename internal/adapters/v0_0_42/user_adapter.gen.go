...
// UserAdapter implements the User adapter interface for v0_0_42
...
func NewUserAdapter(client *api.ClientWithResponses) *UserAdapter {
	return &UserAdapter{
		BaseManager: adapterbase.NewBaseManager("v0.0.42", "User"),
		client:      client,
	}
}
...
func (a *UserAdapter) Get(ctx context.Context, userName string) (*types.User, error) {
	...
	// Call the API
	resp, err := a.client.SlurmdbV0042GetUserWithResponse(ctx, userName, params)
	if err != nil {
		return nil, a.HandleAPIError(err)
	}
...
func (a *UserAdapter) Create(ctx context.Context, input *types.UserCreate) (*types.UserCreateResponse, error) {
	...
	reqBody := api.SlurmdbV0042PostUsersJSONRequestBody{
		Users: []api.V0042User{*apiInput},
	}

	// Call the API
	resp, err := a.client.SlurmdbV0042PostUsersWithResponse(ctx, reqBody)
	if err != nil {
		return nil, a.HandleAPIError(err)
	}
...
func (a *UserAdapter) Delete(ctx context.Context, userName string) error {
	...
	// Call the API
	resp, err := a.client.SlurmdbV0042DeleteUserWithResponse(ctx, userName)
	if err != nil {
		return a.HandleAPIError(err)
	}
