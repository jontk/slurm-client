...
func (a *JobAdapter) holdJobImpl(ctx context.Context, req *types.JobHoldRequest) error {
	...
	resp, err := a.client.SlurmV0042PostJobWithResponse(ctx, strconv.Itoa(int(req.JobId)), updateReq)
	if err != nil {
...
func (a *JobAdapter) signalJobImpl(ctx context.Context, req *types.JobSignalRequest) error {
	...
	resp, err := a.client.SlurmV0042DeleteJobWithResponse(ctx, strconv.Itoa(int(req.JobId)), params)
	if err != nil {
		return a.HandleAPIError(err)
	}
...
func (a *JobAdapter) notifyJobImpl(ctx context.Context, req *types.JobNotifyRequest) error {
	...
	return errors.NewClientError(
		...
		"Job notification is not supported via REST API v0.0.42. Use 'scontrol notify' command instead",
...
func (a *JobAdapter) allocateJobImpl(ctx context.Context, req *types.JobAllocateRequest) (*types.JobAllocateResponse, error) {
	...
	if req.Memory != "" {
		...
		if err == nil && memVal > 0 {
			memPerNode := &api.V0042Uint64NoValStruct{}
	...
	if len(req.Command) > 0 {
		cmdArray := &api.V0042StringArray{}
		*cmdArray = append(*cmdArray, req.Command...)
		jobDesc.Argv = cmdArray
	}

	allocReq := api.V0042JobAllocReq{
		Job: jobDesc,
	}

	// Call the API
	resp, err := a.client.SlurmV0042PostJobAllocateWithResponse(ctx, allocReq)
	if err != nil {
	...
	responseAdapter := api.NewResponseAdapter(resp.StatusCode(), apiErrors)
	if err := common.HandleAPIResponse(responseAdapter, "V0042"); err != nil {
		return nil, err
	}
...
func (a *JobAdapter) convertCommonJobUpdateToAPIRequestBody(update *types.JobUpdate) api.SlurmV0042PostJobJSONRequestBody {
	if update == nil {
		return api.SlurmV0042PostJobJSONRequestBody{}
	}
...
func (a *JobAdapter) convertAPIJobSubmitResponseToCommon(resp *api.V0042OpenapiJobSubmitResponse) *types.JobSubmitResponse {
	if resp == nil {
		return nil
	}
...
func (a *JobAdapter) convertAPIJobAllocateResponseToCommon(resp *api.V0042OpenapiJobAllocResp) *types.JobAllocateResponse {
	if resp == nil {
		return nil
	}
