...
func (a *QoSAdapter) enhanceQoSWithSkippedFields(result *types.QoS, apiObj api.V0042Qos) {
	if result == nil {
		return
	}
	// Flags
	if apiObj.Flags != nil {
		for _, flag := range *apiObj.Flags {
			result.Flags = append(result.Flags, types.QoSFlagsValue(flag))
		}
	}
	// Limits
...
func convertAPIQoSPreemptToCommon(apiPreempt *struct {
	...
	// Mode - In v0_0_42, Mode is a []string, not an enum slice
...
func convertAPITresListToCommon(apiTres api.V0042TresList) []types.TRES {
	if len(apiTres) == 0 {
		return nil
	}
