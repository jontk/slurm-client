...
func (m *PartitionBaseManager) matchesPartitionFilters(partition types.Partition, opts *types.PartitionListOptions) bool {
	...
	partitionStates := getPartitionStates(&partition)
	// Filter by names
	if len(opts.Names) > 0 {
		found := false
	...
	// Filter by states - check if any partition state matches any filter state
	if len(opts.States) > 0 {
		found := false
		for _, filterState := range opts.States {
			for _, partState := range partitionStates {
				if partState == filterState {
					found = true
					break
				}
			}
