...
// NewUserAdapter creates a new User adapter for v0.0.44
func NewUserAdapter(client *api.ClientWithResponses) *UserAdapter {
	return &UserAdapter{
		BaseManager: base.NewBaseManager("v0.0.44", "User"),
		client:      client,
...
func (a *UserAdapter) List(ctx context.Context, opts *types.UserListOptions) (*types.UserList, error) {
	...
	if opts != nil {
		// Note: SlurmdbV0044GetUsersParams doesn't have a Name field for filtering
...
func (a *UserAdapter) convertAPIUserToCommon(apiUser api.V0044User) (*types.User, error) {
	...
	if apiUser.Flags != nil {
		for _, flag := range *apiUser.Flags {
			if flag == api.V0044UserFlagsDELETED {
				user.Deleted = true
			}
		}
	}
...
func (a *UserAdapter) convertCommonUserAssociationToAPI(req *types.UserAssociationRequest) ([]api.V0044Assoc, error) {
	...
	for _, userName := range req.Users {
		...
		// TODO: DefaultWCKey and AdminLevel are not available in V0044Assoc
