...
// JobAdapter implements the Job adapter interface for v0_0_44
...
func NewJobAdapter(client *api.ClientWithResponses) *JobAdapter {
	return &JobAdapter{
		BaseManager: adapterbase.NewBaseManager("v0.0.44", "Job"),
		client:      client,
	}
}
...
func (a *JobAdapter) Submit(ctx context.Context, job *types.JobCreate) (*types.JobSubmitResponse, error) {
	...
	// Call the API
	resp, err := a.client.SlurmV0044PostJobSubmitWithResponse(ctx, apiJob)
	if err != nil {
		return nil, a.HandleAPIError(err)
	}
...
func (a *JobAdapter) Cancel(ctx context.Context, jobID int32, opts *types.JobCancelRequest) error {
	...
	resp, err := a.client.SlurmV0044DeleteJobWithResponse(ctx, strconv.Itoa(int(jobID)), nil)
	if err != nil {
		return a.HandleAPIError(err)
	}
