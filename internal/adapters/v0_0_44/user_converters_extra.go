...
func (a *UserAdapter) enhanceUserWithSkippedFields(result *types.User, apiObj api.V0044User) {
	if result == nil {
		return
	}
	// AdministratorLevel
	...
	if apiObj.Default != nil {
		...
		if apiObj.Default.Qos != nil {
			result.Default.QoS = apiObj.Default.Qos
		}
	}
	// Flags
	if apiObj.Flags != nil {
...
func convertAPIAssocShortListToCommon(apiList api.V0044AssocShortList) []types.AssocShort {
	if len(apiList) == 0 {
		return nil
	}
...
func convertAPICoordListToCommon(apiList api.V0044CoordList) []types.Coord {
	if len(apiList) == 0 {
		return nil
	}
...
func convertAPIWckeyListToCommon(apiList api.V0044WckeyList) []types.WCKey {
	if len(apiList) == 0 {
		return nil
	}
...
func convertAPIAccountingListToCommon(apiList api.V0044AccountingList) []types.Accounting {
	if len(apiList) == 0 {
		return nil
	}
