...
// NewClusterAdapter creates a new Cluster adapter for v0.0.44
func NewClusterAdapter(client *api.ClientWithResponses) *ClusterAdapter {
	return &ClusterAdapter{
		BaseManager: base.NewBaseManager("v0.0.44", "Cluster"),
		client:      client,
	}
}
...
func (a *ClusterAdapter) Create(ctx context.Context, cluster *types.ClusterCreate) (*types.ClusterCreateResponse, error) {
	...
	if len(cluster.Flags) > 0 {
		// Convert []string to []V0044ClusterRecFlags
		flags := make([]api.V0044ClusterRecFlags, len(cluster.Flags))
		for i, flag := range cluster.Flags {
			flags[i] = api.V0044ClusterRecFlags(flag)
		}
		apiCluster.Flags = &flags
	}

	// Create request body
	apiReq := api.V0044OpenapiClustersResp{
		Clusters: []api.V0044ClusterRec{*apiCluster},
	}

	// Call the API
	params := &api.SlurmdbV0044PostClustersParams{}
	resp, err := a.client.SlurmdbV0044PostClustersWithResponse(ctx, params, apiReq)
	if err != nil {
	...
	// Convert response - v0.0.44 POST returns V0044OpenapiResp not V0044OpenapiClustersResp
...
func (a *ClusterAdapter) Delete(ctx context.Context, clusterName string) error {
	...
	// Prepare parameters for the API call
	params := &api.SlurmdbV0044DeleteClusterParams{}
...
func (a *ClusterAdapter) convertAPIClusterToCommon(apiCluster api.V0044ClusterRec) (*types.Cluster, error) {
	...
	if apiCluster.Flags != nil {
		// Convert []V0044ClusterRecFlags to []string
...
func (a *ClusterAdapter) convertAPIClusterCreateResponseToCommon(apiResp *api.V0044OpenapiResp, name string) (*types.ClusterCreateResponse, error) {
	...
	// V0044OpenapiResp doesn't contain clusters - it's a general response
	// We cannot extract details from the response for v0.0.44
	...
	// Handle errors in response - V0044OpenapiErrors is []V0044OpenapiError
...
func (a *ClusterAdapter) extractMeta(meta *api.V0044OpenapiMeta) map[string]interface{} {
	...
	// v0.0.44 doesn't have Messages, Warnings, or Errors in meta
