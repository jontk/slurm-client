...
func TestAssociationAdapter_Get(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "Association name is required")
...
func TestAssociationAdapter_Update(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "update is required")
...
func TestAssociationAdapter_GetDefaultClusterName(t *testing.T) {
	...
	// Default should be "linux" as per implementation
	assert.Equal(t, "linux", clusterName)
}
