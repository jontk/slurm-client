...
func (u *TRESUtils) ConvertAPITRESToCommon(apiTresList api.V0044TresList) []types.TRES {
	if apiTresList == nil {
		return []types.TRES{}
	}
...
func (u *TRESUtils) ConvertCommonTRESToAPI(tresList []types.TRES) api.V0044TresList {
	if len(tresList) == 0 {
		return api.V0044TresList{}
	}
	apiTresList := make(api.V0044TresList, 0, len(tresList))
	for _, tres := range tresList {
		apiTres := api.V0044Tres{
			Type: tres.Type,
		}
