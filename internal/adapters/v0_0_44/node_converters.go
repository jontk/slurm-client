...
func (a *NodeAdapter) convertCommonNodeUpdateToAPI(existing *types.Node, update *types.NodeUpdate) *api.V0044Node {
	...
	if update.NextStateAfterReboot != nil {
		nextStates := []api.V0044NodeNextStateAfterReboot{api.V0044NodeNextStateAfterReboot(*update.NextStateAfterReboot)}
	...
	if update.ResumeAfter != nil {
		...
		apiNode.ResumeAfter = &api.V0044Uint64NoValStruct{
			Set:    &setTrue,
			Number: &resumeAfter,
		}
	}
	...
	// State
	if update.State != nil {
		states := []api.V0044NodeState{api.V0044NodeState(*update.State)}
