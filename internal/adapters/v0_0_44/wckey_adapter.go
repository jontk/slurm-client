...
// WCKeyAdapter implements the WCKeyAdapter interface for v0.0.44
...
// NewWCKeyAdapter creates a new WCKey adapter for v0.0.44
func NewWCKeyAdapter(client *api.ClientWithResponses) *WCKeyAdapter {
	return &WCKeyAdapter{
		BaseManager:  base.NewBaseManager("v0.0.44", "WCKey"),
		client:       client,
...
func (a *WCKeyAdapter) Get(ctx context.Context, wcKeyID string) (*types.WCKey, error) {
	...
	// Call the API
	resp, err := a.client.SlurmdbV0044GetWckeyWithResponse(ctx, wcKeyID)
	if err != nil {
...
func (a *WCKeyAdapter) Create(ctx context.Context, wckey *types.WCKeyCreate) (*types.WCKeyCreateResponse, error) {
	...
	apiReq := api.V0044OpenapiWckeyResp{
		Wckeys: api.V0044WckeyList{apiWCKey},
	}

	// Create params (can be nil for this endpoint)
	params := &api.SlurmdbV0044PostWckeysParams{}

	// Call the API
	resp, err := a.client.SlurmdbV0044PostWckeysWithResponse(ctx, params, apiReq)
	if err != nil {
	...
	if resp.JSON200 != nil {
		...
		// Note: V0044OpenapiResp doesn't contain the created WCKey details
...
func (a *WCKeyAdapter) Delete(ctx context.Context, wcKeyID string) error {
	...
	// Call the API
	resp, err := a.client.SlurmdbV0044DeleteWckeyWithResponse(ctx, wcKeyID)
	if err != nil {
...
func (a *WCKeyAdapter) convertAPIWCKeyToCommon(apiWCKey api.V0044Wckey) types.WCKey {
	wckey := types.WCKey{
		Meta: make(map[string]interface{}),
	}
...
func (a *WCKeyAdapter) convertCommonWCKeyCreateToAPI(wckey *types.WCKeyCreate) api.V0044Wckey {
	// Name, User, and Cluster are required string fields in V0044Wckey
	apiWCKey := api.V0044Wckey{
		Name:    wckey.Name,
		User:    wckey.User,
...
func (a *WCKeyAdapter) extractMeta(meta *api.V0044OpenapiMeta) map[string]interface{} {
