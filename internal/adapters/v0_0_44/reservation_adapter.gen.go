...
// ReservationAdapter implements the Reservation adapter interface for v0_0_44
...
func NewReservationAdapter(client *api.ClientWithResponses) *ReservationAdapter {
	return &ReservationAdapter{
		BaseManager: adapterbase.NewBaseManager("v0.0.44", "Reservation"),
		client:      client,
	}
}
...
func (a *ReservationAdapter) Create(ctx context.Context, input *types.ReservationCreate) (*types.ReservationCreateResponse, error) {
	...
	// Convert to API type (Reservation uses V0044ReservationDescMsg for input)
	apiInput := a.convertCommonReservationCreateToAPI(input)

	// Create request body - Reservation uses a different structure
	list := api.V0044ReservationDescMsgList{*apiInput}
	reqBody := api.SlurmV0044PostReservationsJSONRequestBody{
		Reservations: &list,
	}

	// Call the API
	resp, err := a.client.SlurmV0044PostReservationsWithResponse(ctx, reqBody)
	if err != nil {
		return nil, a.HandleAPIError(err)
	}
