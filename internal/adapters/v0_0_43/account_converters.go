...
func (a *AccountAdapter) convertAPIAccountToCommon(apiAccount api.V0043Account) (*types.Account, error) {
	...
	// Note: The v0.0.43 API Account struct only contains:
...
func (a *AccountAdapter) convertCommonAccountCreateToAPI(create *types.AccountCreate) (*api.V0043Account, error) {
	...
	if len(create.Coordinators) > 0 {
		coordinators := make([]api.V0043Coord, len(create.Coordinators))
		for i, coord := range create.Coordinators {
			coordinators[i] = api.V0043Coord{
				Name: coord,
			}
		}
	...
	// Note: v0.0.43 API Account struct only supports basic fields:
...
func (a *AccountAdapter) convertCommonAccountUpdateToAPI(existing *types.Account, update *types.AccountUpdate) (*api.V0043Account, error) {
	...
	if len(coordinators) > 0 {
		apiCoordinators := make([]api.V0043Coord, len(coordinators))
		for i, coord := range coordinators {
			apiCoordinators[i] = api.V0043Coord{
				Name: coord,
			}
		}
