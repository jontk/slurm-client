...
func (a *JobAdapter) convertCommonJobCreateToAPI(input *types.JobCreate) api.SlurmV0043PostJobSubmitJSONRequestBody {
	if input == nil {
		return api.SlurmV0043PostJobSubmitJSONRequestBody{}
	}
	...
	if input.TimeLimit != nil {
		jobDesc.TimeLimit = &api.V0043Uint32NoValStruct{
			Set:    ptrBool(true),
	...
	if input.MemoryPerNode != nil {
		jobDesc.MemoryPerNode = &api.V0043Uint64NoValStruct{
			Set:    ptrBool(true),
			Number: ptrInt64(int64(*input.MemoryPerNode)),
		}
	}
	if input.MemoryPerCPU != nil {
		jobDesc.MemoryPerCpu = &api.V0043Uint64NoValStruct{
			Set:    ptrBool(true),
	...
	if len(input.Environment) > 0 {
		envArray := api.V0043StringArray(input.Environment)
