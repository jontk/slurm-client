...
func (a *JobAdapter) holdJobImpl(ctx context.Context, req *types.JobHoldRequest) error {
	...
	resp, err := a.client.SlurmV0043PostJobWithResponse(ctx, strconv.Itoa(int(req.JobId)), updateReq)
	if err != nil {
...
func (a *JobAdapter) signalJobImpl(ctx context.Context, req *types.JobSignalRequest) error {
	...
	resp, err := a.client.SlurmV0043DeleteJobWithResponse(ctx, strconv.Itoa(int(req.JobId)), params)
	if err != nil {
		return a.HandleAPIError(err)
	}
...
func (a *JobAdapter) notifyJobImpl(ctx context.Context, req *types.JobNotifyRequest) error {
	...
	return errors.NewClientError(
		...
		"Job notification is not supported via REST API v0.0.43. Use 'scontrol notify' command instead",
...
func (a *JobAdapter) allocateJobImpl(ctx context.Context, req *types.JobAllocateRequest) (*types.JobAllocateResponse, error) {
	...
	if req.Memory != "" {
		...
		if err == nil && memVal > 0 {
			memPerNode := &api.V0043Uint64NoValStruct{}
	...
	if req.TimeLimit > 0 {
		timeLimit := &api.V0043Uint32NoValStruct{}
	...
	if len(req.Command) > 0 {
		cmdArray := &api.V0043StringArray{}
		*cmdArray = append(*cmdArray, req.Command...)
		jobDesc.Argv = cmdArray
	}

	allocReq := api.V0043JobAllocReq{
		Job: jobDesc,
	}

	// Call the API
	resp, err := a.client.SlurmV0043PostJobAllocateWithResponse(ctx, allocReq)
	if err != nil {
	...
	responseAdapter := api.NewResponseAdapter(resp.StatusCode(), apiErrors)
	if err := common.HandleAPIResponse(responseAdapter, "V0043"); err != nil {
		return nil, err
	}
...
func (a *JobAdapter) convertCommonJobUpdateToAPIRequestBody(update *types.JobUpdate) api.SlurmV0043PostJobJSONRequestBody {
	if update == nil {
		return api.SlurmV0043PostJobJSONRequestBody{}
	}
...
func (a *JobAdapter) convertAPIJobSubmitResponseToCommon(resp *api.V0043OpenapiJobSubmitResponse) *types.JobSubmitResponse {
	if resp == nil {
		return nil
	}
...
func (a *JobAdapter) convertAPIJobAllocateResponseToCommon(resp *api.V0043OpenapiJobAllocResp) *types.JobAllocateResponse {
	if resp == nil {
		return nil
	}
